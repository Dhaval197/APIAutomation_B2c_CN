...
class TestZFBpayProfess(object):
    ...
    def test_016_create_Order(self, get_load_data_oms, get_url, i=14):
        ...
        globals()['orderCode1'] = response.json().get('data').get('orderCode').strip()
    ...
    @pytest.mark.parametrize("get_load_data_oms, get_url", [((path, current_file), '')],
                             indirect=["get_load_data_oms", "get_url"])
    def test_019_alipay(self,get_load_data_oms, get_url, eurl, i=19,):
        """支付宝支付"""
        allure.dynamic.title("19选择支付宝支付")
        if eurl == 'cn_test':
            zfb_url = 'https://openapi-cn-test.xiaoyi.com/orderpay/v8/ali/pay/callback'  # 支付宝测试，不走网关 ,测试环境
        else:
            zfb_url = "https://open-gw.xiaoyi.com/orderpay/v8/ali/pay/callback"  # 支付宝正式环境，不走网关 ，正式环境
        alipay = AliPay(

            appid="2021000119608939",  # KAMI 创建的沙箱环境的appid
            # appid="2016102400750431",  # 就得创建的沙箱环境的appid
            app_notify_url=zfb_url,  # 设置为后台回调地址
        ...
        # 输入账号
        self.driver.find_element_by_xpath('//*[@id="J_tLoginId"]').send_keys('edfqrk0301@sandbox.com')
        # self.driver.find_element_by_xpath('//*[@id="J_tLoginId"]').send_keys('meassh9032@sandbox.com')
        time.sleep(1)
        # 输入密码
        self.driver.find_element_by_xpath('//*[@id="payPasswd_rsainput"]').send_keys('111111')
        time.sleep(1)
        # 点击下一步
        self.driver.find_element_by_xpath('//*[@id="J_newBtn"]/span').click()
        time.sleep(5)
        # 获取所有的窗口句柄
        ...
        time.sleep(3)
        # 输入支付宝支付密码
        self.driver.find_element_by_id('payPassword_rsainput').send_keys('111111')
        time.sleep(3)
        # 点击确认按钮
