...
# 沙箱环境中 app 私钥
app_private_key_string = open('app_private_key20220210.pem').read()

# alipay_public_key.pem   支付宝公钥
alipay_public_key_string = open('alipay_public_key20220210.pem').read()
...
class TestZFBpayProfess(object):
    ...
    def test_004_oms_getSkuChannelPage(self, get_load_data_oms, eurl, i=3):
        ...
        logger.info('请求参数为：' + str(payload))
        headers = {'Authorization': 'Bearer ' + globals()['access_token1']}
        for m in range(1, 100):  # 查看100页的数据
            falg = 0
            try:
                payload['page'] = m
